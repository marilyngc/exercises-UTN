...
class App(customtkinter.CTk):
    ...
    def btn_cantidad_camiones_on_click(self):
        toneladas_ingresadas = self.txt_toneladas.get();
        toneladas_ingresadas = int(toneladas_ingresadas);
        
        toneladas_a_kilogramos = (toneladas_ingresadas * 1000) + 1750;
        cantidad_camiones = toneladas_a_kilogramos / 3500;
        cantidad_camiones = round(cantidad_camiones);
        
        alert("cantidad de camiones", f"la cantidad de camiones necesarios son: {cantidad_camiones}");
        
    def btn_tiempo_llegada_on_click(self):
        kilómetros_ingresados = self.txt_kilometros.get();
        kilómetros_ingresados = int(kilómetros_ingresados);
        
        
        
        tiempo_camiones = kilómetros_ingresados / 90;
        alert("tiempo de llegada", f"los camiones llegan en: {tiempo_camiones} hora");
